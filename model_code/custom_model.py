...
class CustomRain(nn.Module):
    ...
    def forward(self, stack):
        ...
        # axial_l1_l2 = self.axial_l1_l2_backbone(axial_l1_l2)
        # axial_l2_l3 = self.axial_l1_l2_backbone(axial_l2_l3)
        # axial_l3_l4 = self.axial_l1_l2_backbone(axial_l3_l4)
        # axial_l4_l5 = self.axial_l1_l2_backbone(axial_l4_l5)
        # axial_l5_s1 = self.axial_l1_l2_backbone(axial_l5_s1)
        x = self.sagittal_l1_l2_backbone(stack)
        ...
        # x4 = self.l4_l5(x4)
        # x5 = self.l5_s1(x5)
        # x = torch.cat([axial_l1_l2, sagittal_T2_l1_l2, axial_l2_l3, sagittal_T2_l2_l3, axial_l3_l4, sagittal_T2_l3_l4, axial_l4_l5, sagittal_T2_l4_l5, axial_l5_s1, sagittal_T2_l5_s1], dim=1)
        # x = self.fc(sagittal_T2_l1_l2)
        # x = torch.cat([x1, x2, x3, x4, x5], dim=1)
...
class MultiTaskModelDifferentInputs(nn.Module):
    def __init__(self,num_classes=15, pretrained=True):
        super(MultiTaskModelDifferentInputs, self).__init__()

        # Define the axial backbone using TIMM's ViT model
        ...
        # Define the sagittal backbone using TIMM's EdgeNeXt model
        self.sagittal_l1_l2_backbone = timm.create_model(
            "timm/edgenext_base.in21k_ft_in1k",
            pretrained=pretrained, 
            features_only=False,
        ...
        # Define task-specific heads for each pair of inputs
        self.task_specific_heads = nn.ModuleList([
            nn.Sequential(nn.Dropout(0.3),
            nn.GELU(),
            nn.Linear(256, num_classes)
            ) for _ in range(5)
        ])
    ...
    def forward(self, *input_pairs):
        outputs = []
        for (axial, sagittal), head in zip(input_pairs, self.task_specific_heads):
            # Pass through respective backbones
            axial_features = self.axial_backbone(axial)
            sagittal_features = self.sagittal_l1_l2_backbone(sagittal)
        ...
        # Stack the outputs and then reshape if needed
        outputs = torch.stack(outputs)  # This will create a tensor with shape (5, batch_size, num_classes)
        outputs = outputs.view(-1, 15 * 5)  # Flattening
        return outputs